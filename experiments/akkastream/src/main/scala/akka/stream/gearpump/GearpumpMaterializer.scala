    ...
    val session = new MaterializerSession(runnableGraph.module, initialAttributes) {
      ...
      protected override def materializeModule(module: Module, parentAttributes: Attributes): Any = {
        ...
        for (submodule <- module.subModules) {
          submodule match {
            case atomic: AtomicModule if atomic.isAtomic =>
      ...
      protected def materializeAtomic(atomic: AtomicModule, parentAttributes: Attributes, matVal: ju.Map[Module, Any]): Unit = {
